...
End-to-end workflow for continued pretraining Qwen3.5 as a text-only `GPTModel` on AMD MI325X.  Validated on **Qwen3.5-4B-Base**.  All tooling lives in `tw-tools/`.

**Prerequisite — Megatron-LM fork.** The TW tools require the [OpenEuroLLM/NVIDIA-Megatron-LM `head_nvidia` branch](https://github.com/OpenEuroLLM/NVIDIA-Megatron-LM/tree/head_nvidia), which carries AMD/ROCm fixes and the GDN layer support needed for Qwen3.5.  Clone it and update `MEGATRON_PATH_ABS` in `tw-tools/import_hf_to_megatron.sh` to point at your checkout.

Set these shell variables once and reuse them across all stages:

```bash
HF_CKPT=Qwen/Qwen3.5-4B-Base          # HF hub id or local path
MEG_CKPT=megatron_ckpt/Qwen3.5-4B-Base # Megatron checkpoint directory
OUT_DIR=hf_export/Qwen3.5-4B-Base      # final HF export directory

cd /path/to/Megatron-Bridge
mkdir -p logs-convert
```

### Stage 1 — HuggingFace → Megatron checkpoint

```bash
sbatch tw-tools/import_hf_to_megatron.sh \
    --hf-model "$HF_CKPT" \
    --megatron-path "$MEG_CKPT"
```

Always produces a text-only `GPTModel` (vision tower stripped).  For MoE models override resources on the `sbatch` line (e.g. `--ntasks=8 --gres=gpu:mi325:8 --mem=512G`) and pass `--expert-model-parallel-size 8`.

**Required Megatron-LM training flags** to match the bridge's RMSNorm convention (`config/backend/megatron/qwen3_5_base.yaml`):
...
### Stage 2 — Verify the checkpoint (optional)

```bash
sbatch tw-tools/verify_text_ckpt.sh "$HF_CKPT" "$MEG_CKPT"
```

### Stage 3 — Continued pretraining

Run `pretrain_gpt.py` with your data and `$MEG_CKPT` as `--load`.  See `oellm-autoexp/output/qwen3_5_4B_tw_test/job.sbatch` for a working reference.

### Stage 4 — Export trained weights back to HuggingFace
...
### Stage 5 — Verify the roundtrip (optional)

```bash
sbatch tw-tools/compare_roundtrip.sh "$HF_CKPT" "$OUT_DIR"
```

Compares all parameter groups (vision, merger, lm) between the original and the exported checkpoint, reporting max absolute difference and cosine similarity per key.
